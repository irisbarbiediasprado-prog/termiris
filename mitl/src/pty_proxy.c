...
static protocol_extractor_t extractor;
static terminal_emulator_t terminal;
...
static void on_pty_master_read(int fd, void *arg) {
    ...
    if (proxy->sink.on_output) {
        protocol_extractor_scan(
            &extractor,
            terminal_emulator_visible_text(&terminal),
            proxy->sink.on_output,
            proxy->sink.user_data
        );
    }
}
...
static void on_stdin_read(int fd, void *arg) {
    ...
    if (n > 0)
        (void)write(proxy->master_fd, buf, (size_t)n);
}
...
static void handle_sigwinch(int sig) {
    ...
    if (ioctl(STDIN_FILENO, TIOCGWINSZ, &ws) == 0) {
        ...
        if (g_proxy_ref->sink.on_resize)
            g_proxy_ref->sink.on_resize(
                ws.ws_row,
                ws.ws_col,
                g_proxy_ref->sink.user_data
            );
    }
}
...
int pty_proxy_init(
    ...
    protocol_extractor_init(&extractor);
    terminal_emulator_init(&terminal);

    struct winsize ws;
    ioctl(STDIN_FILENO, TIOCGWINSZ, &ws);

    proxy->child_pid = forkpty(
        &proxy->master_fd,
        NULL,
        NULL,
        &ws
